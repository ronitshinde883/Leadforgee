...
def login_and_csrf_exempt(view):
    return method_decorator([csrf_exempt, login_is_required])(view)


@login_and_csrf_exempt
...
class DealDetailView(View):
    def get(self, request, deal_id):
        try:
            ...
            if deal.company != company:
                return JsonResponse({"error": "Cannot access other company's data"}, status=400)
    ...
    def post(self, request, deal_id):
        try:
            ...
            if deal.company != company:
                return JsonResponse({"error": "Cannot modify other company's data"}, status=401)
...
class DealDeleteView(View):
    def post(self, request, deal_id):
        try:
            ...
            if deal.company != company:
                return JsonResponse({"error": "Cannot delete other company's data"}, status=401)

            deal.delete()
            return JsonResponse({"message": "Deal deleted successfully"}, status=200)
...
class DealReactivateView(View):
    def post(self, request, deal_id):
        try:
            ...
            if deal.company != company:
                return JsonResponse({"error": "Cannot reactivate other company's data"}, status=401)

            deal.is_active = True
            deal.save()
            return JsonResponse({"message": "Deal reactivated successfully"}, status=200)
